...
from utils.data_loader import load_data_from_excel, DATA_FILE_PATH 
...
def linear_regression(x, y):
    ...
    se_pred = se * np.sqrt(1+1/n + (x - x_mean)**2 / sum_of_squares_x) # prediction interval has a 1+1/n+ term, CI its just 1/n + term
    ci = t_value * se_pred
    upper_ci = y_pred + ci
    lower_ci = y_pred - ci
...
st.cache_data
def load_data():
    df, DATA_FILE_PATH = load_data_from_excel() # This line is correct
    # This function should load your data and return the DataFrame
    return df, DATA_FILE_PATH # This function is returning (df, path)

df, CSV_FILE_PATH = load_data() 

if "TFF_df" not in st.session_state:
    try:
        st.session_state.TFF_df = df
        
        # Store the correct path for saving
        st.session_state.data_file_path = DATA_FILE_PATH
        
        # Initialize session state for filters
        st.session_state.filters = {col: [] for col in st.session_state.TFF_df.columns}
...
def save_full_dataframe(edited_df, file_path):
    """Saves the entire edited DataFrame back to the Excel (.xlsx) file."""
    
    # 💥 FIX: Use to_excel to maintain the original file format
    # CRITICAL: Specify the sheet_name to ensure data is written to the correct tab.
    # Replace 'Sheet1' with the actual name of your Excel tab if it's different.
    edited_df.to_excel(file_path, index=False, sheet_name='reference')
    
    # Update the session state copy
    st.session_state.TFF_df = edited_df.copy()

    # Reset filters and trigger rerun
    st.session_state.filters = {col: [] for col in st.session_state.TFF_df.columns}
...
st.title("TFF Valve Status Check")

# Sidebar Mode Selector
st.sidebar.header("Application Mode")
app_mode = st.sidebar.radio(
    "Select Mode", 
    ("TFF Status Check", "Edit TFF Data"), 
    index=0 # Default to view mode
)

# Use the full data from session state
df = st.session_state.TFF_df.copy()

# --- EDIT RAW DATA MODE ---
if app_mode == "Edit TFF Data":
    st.subheader("Edit Original TFF Data")
...
elif app_mode == "TFF Status Check":
